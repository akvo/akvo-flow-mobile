...
class CreateFormInstance @Inject constructor(
    ...
    private fun <T> buildUseCaseObservable(parameters: Map<String, T>): Single<Long> {
        if (!parameters.containsKey(PARAM_FORM_INSTANCE) ) {
            return Single.error(IllegalArgumentException("Missing form id"))
        }
        val domainFormInstance = parameters[PARAM_FORM_INSTANCE] as DomainFormInstance
        return formInstanceRepository.createFormInstance(domainFormInstance)
    }
    ...
    companion object {
        const val PARAM_FORM_INSTANCE = "form_instance"
    }
}
