...
public class SurveyDataRepository implements SurveyRepository {
    ...
    private Observable<Integer> syncDataPoints(final long surveyGroupId) {
        ...
        return loadAndSave(surveyGroupId, lastBatch, allResults, syncedTime)
    ...
    private Observable<List<ApiDataPoint>> loadAndSave(final long surveyGroupId,
            final List<ApiDataPoint> lastBatch, final List<ApiDataPoint> allResults,
            String syncedTime) {
        return loadNewDataPoints(surveyGroupId, syncedTime, lastBatch)
    ...
    private Observable<ApiLocaleResult> loadNewDataPoints(final long surveyGroupId,
            final String syncedTime, final List<ApiDataPoint> lastBatch) {
                        ...
                        return getLatestSyncedTime(syncedTime, lastBatch)
                                .flatMap(new Function<String, Observable<ApiLocaleResult>>() {
                                    ...
                                    public Observable<ApiLocaleResult> apply(String time) {
                                        return restApi
                                                .loadNewDataPoints(surveyGroupId, time);
                                    }
                                });
                    }
                });
