...
public class SubmittedSurveyReviewCursorAdaptor extends CursorAdapter {
    ...
    /**
     * Gets status of latest transmission for this respondent. This will usually
     * be the zip file, as it is sent after any media files
     */
    ...
    /**
     * Gets any non-sent status of any non-zip transmission for this respondent,
     * or DETAIL_NONE if all succeeded/none can be found. We need to ignore any
     * files that have been successful at any time
     */
    int getWorstMediaTransmissionStatus(Long respondId) {
        databaseAdapter.open();
        // get file transmissions, most recent first, including successes
        List<FileTransmission> transList = databaseAdapter.listFileTransmission(respondId, null,
                false);
        int sts = DETAIL_NONE;
        if (transList != null) {
            // first pass - mark any files that have been successful at any time
            // as successful
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (millis != 0) {
            ...
            if (detail != DETAIL_NONE) { // problems
                ...
                status = "Sent (not photos): "; // TODO: move to string resource
                                                // for localization
            }
